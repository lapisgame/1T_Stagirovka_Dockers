...
class zarplata_parser:
    def __init__(self, max_page_count=3) -> None:
        self.table_name = 'zarplata_row'
    ...
    def set_actual(self):
        ...
        sql = "INSERT INTO zarplata_row VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    ...
    def pars_vac(self, vac_name:str, index:int):
        for page_number in range(self.max_page_count):
            ...
            try:
                ...
                req = requests.get('https://api.zarplata.ru/vacancies', params=params).json()
                time.sleep(5)
                
                if 'items' in req.keys():
                    for item in req['items']:
                        item = requests.get(f'https://api.zarplata.ru/vacancies/{item["id"]}').json()
                        res = {}
                        try:
                            res['vacancy_id'] = f'https://www.zarplata.ru/vacancy/card/id{item["id"]}'
                            ...
                            logging.error(f'В процессе парсинга вакансии https://www.zarplata.ru/vacancy/card/id{item["id"]} произошла ошибка {exc} \n\n')
...
dag = DAG(
    'ZARPLATA_parsing',
    description='DAG для парсинга вакансий zarplata',
    schedule="@daily",
    start_date=datetime(2023, 11, 28)
)

parser = zarplata_parser()

Create_table = PostgresOperator(
    task_id="create_zarplata_row_table",
    sql="""
        CREATE TABLE IF NOT EXISTS zarplata_row (
